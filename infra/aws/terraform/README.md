...
It is intentionally small and reviewable: no third-party Terraform modules, no NAT gateway, public ALB, public-IP ECS tasks, and private RDS subnets.

## What it creates

- ECR repository for the app image.
- VPC with two public subnets, two private DB subnets, an internet gateway, and no NAT gateway.
- Public ALB with `/healthz` target-group checks on container port `4040`.
- ACM DNS-validated certificate outputs for manual Cloudflare records.
- ECS Fargate cluster, web service, and singleton scheduler service.
- Web task env: `PROCESS_ROLE=web`, `RUN_SCHEDULER=false`.
...
It runs only from the `production` branch or manual `workflow_dispatch`, assumes the `github_deploy_role_arn` role through OIDC, pushes an immutable image tag to ECR, runs `yarn db:migrate` as a one-off ECS task, then updates the web and scheduler services.
